...
exports.BudgetOptimizer = void 0;
class BudgetOptimizer {
     ...
     * @returns 최적화 결과
     */
    static solveBinaryKnapsack(items, constraint) {
        const { totalBudget, mandatoryItems = [], excludedItems = [] } = constraint;
        // 1. 필터링: 제외 항목 제거, 예산 초과 항목 제거
        const validItems = items.filter(item => !excludedItems.includes(item.alternativeId) &&
            item.cost <= totalBudget);
        // 2. 필수 항목 처리
        const mandatoryItemsData = validItems.filter(item => mandatoryItems.includes(item.alternativeId));
        ...
        const optionalItems = validItems.filter(item => !mandatoryItems.includes(item.alternativeId));
     ...
     * @returns 최적화 결과
     */
    static solveContinuousAllocation(items, constraint) {
        ...
        for (const item of sortedItems) {
            ...
            const maxAllocation = Math.min(maxBudgetPerItem || item.cost, item.cost, remainingBudget);
            let allocation = 0;
     ...
     * @returns 시나리오 분석 결과
     */
    static performScenarioAnalysis(items, baseConstraint, scenarios = [-20, -10, 10, 20], isBinary = false) {
        // 기본 시나리오 계산
     ...
     * @returns DP 테이블
     */
    static solveKnapsackDP(items, budget) {
     ...
     * @returns 선택된 항목들
     */
    static backtrackKnapsack(items, budget, dp) {
        const n = items.length;
     ...
     * @returns 추천 항목들
     */
    static getEfficiencyRecommendations(items, topK = 5) {
        return [...items]
     ...
     * @returns 인사이트 메시지들
     */
    static generateInsights(result, items) {
        const insights = [];
        // 예산 활용률 분석
        ...
        const unselectedHighEfficiency = items.filter(item => {
            const isSelected = result.allocations.some(alloc => alloc.alternativeId === item.alternativeId && alloc.isSelected);
...
exports.BudgetOptimizer = BudgetOptimizer;
exports.default = BudgetOptimizer;
