...
exports.AHPCalculatorService = void 0;
/**
 * AHP 계산 서비스 클래스
 */
class AHPCalculatorService {
     ...
     * @returns AHP 결과 객체
     */
    static calculateConsistency(matrix, weights) {
        const n = matrix.length;
        if (n <= 2) {
     ...
     * @returns 개선 제안 목록
     */
    static generateInconsistencyAdvice(matrix, weights, topK = 3) {
        const n = matrix.length;
        if (n <= 2)
            return [];
        try {
            // 1. 일관 행렬 계산: A^_ij = wi / wj
            const consistentMatrix = weights.map(wi => weights.map(wj => wj > 0 ? wi / wj : 1));
     ...
     */
    static findNearestSaatyValue(target) {
        const saatyValues = [1 / 9, 1 / 8, 1 / 7, 1 / 6, 1 / 5, 1 / 4, 1 / 3, 1 / 2, 1, 2, 3, 4, 5, 6, 7, 8, 9];
        return saatyValues.reduce((prev, curr) => Math.abs(curr - target) < Math.abs(prev - target) ? curr : prev);
    }
    /**
     * 직접입력 데이터 정규화
     * @param values 원시값 배열
     ...
     * @returns 정규화된 가중치
     */
    static normalizeDirectInput(values, isBenefit = true) {
        if (values.length === 0)
            return [];
        try {
            // 1. 비용형인 경우 역수 처리
     ...
     * @returns 통합된 가중치
     */
    static synthesizeHierarchy(criteriaWeights, hierarchy) {
        const globalWeights = new Map();
        try {
            // 재귀적으로 계층 구조를 따라 가중치 계산
            function calculateGlobalWeight(criterion, parentWeight = 1.0, path = '') {
                ...
                if (criterion.children && criterion.children.length > 0) {
                    // 하위 기준이 있는 경우
                    criterion.children.forEach((child, index) => {
     ...
     * @returns 통합된 그룹 결과
     */
    static aggregateGroupDecision(individualResults, evaluatorWeights) {
...
exports.AHPCalculatorService = AHPCalculatorService;
// Saaty 척도의 랜덤 일관성 지수 (RI)
AHPCalculatorService.RANDOM_INDEX = [
    0, 0, 0.58, 0.90, 1.12, 1.24, 1.32, 1.41, 1.45, 1.49, 1.51
];
exports.default = AHPCalculatorService;
