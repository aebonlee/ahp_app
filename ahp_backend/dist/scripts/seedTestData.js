...
async function seedTestData(adminUserId) {
    console.log('🌱 Starting test data seeding...');
    try {
        // 1. 프로젝트 생성
        console.log('📊 Creating test project...');
        const projectResult = await (0, connection_1.query)(`INSERT INTO projects (title, description, objective, status, created_by) 
       VALUES ($1, $2, $3, $4, $5) RETURNING id`, [
        ...
        for (const criterion of TEST_DATA.criteria.filter(c => c.level === 1)) {
            const result = await (0, connection_1.query)(`INSERT INTO criteria (name, description, parent_id, level, order_index, project_id, eval_method) 
        ...
        for (const alternative of TEST_DATA.alternatives) {
            const result = await (0, connection_1.query)(`INSERT INTO alternatives (name, description, order_index, project_id) 
        ...
        for (const evaluator of TEST_DATA.evaluators) {
            // 사용자 계정 생성
            const userResult = await (0, connection_1.query)(`INSERT INTO users (email, name, role, password_hash) 
         VALUES ($1, $2, 'evaluator', '$2b$10$dummy_hash') RETURNING id`, [evaluator.email, evaluator.name]);
         ...
         VALUES ($1, $2, $3, $4)`, [projectId, userId, evaluator.code, evaluator.access_key]);
            // 평가자 가중치 설정
            await (0, connection_1.query)(`INSERT INTO evaluator_weights (project_id, evaluator_id, weight) 
         VALUES ($1, $2, $3)`, [projectId, userId, evaluator.weight]);
        ...
        for (const directInput of TEST_DATA.directInputData) {
            ...
            await (0, connection_1.query)(`INSERT INTO direct_entries (project_id, evaluator_id, target_key, value, is_benefit) 
         VALUES ($1, $2, $3, $4, $5)`, [
                projectId,
                evaluatorId,
