...
router.post('/', auth_1.authenticateToken, [
    ...
    (0, express_validator_1.body)('cost').optional().isFloat({ min: 0 }).withMessage('Cost must be non-negative'),
...
router.put('/:id', auth_1.authenticateToken, [
    (0, express_validator_1.param)('id').isInt().withMessage('Alternative ID must be an integer'),
    ...
    try {
        ...
        // 대안 존재 및 접근 권한 확인
        const checkResult = await (0, connection_1.query)(`SELECT a.*, p.admin_id 
         FROM alternatives a
        ...
        // 중복 이름 확인 (수정하는 경우)
        if (updates.name) {
            const duplicateResult = await (0, connection_1.query)('SELECT id FROM alternatives WHERE project_id = ? AND name = ? AND id != ?', [checkResult.rows[0].project_id, updates.name, id]);
            if (duplicateResult.rows.length > 0) {
                return res.status(400).json({ error: 'Alternative with this name already exists' });
            }
        }
