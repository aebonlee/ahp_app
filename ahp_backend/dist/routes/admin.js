...
router.delete('/projects/:id/hard-delete', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/cleanup-phantom-projects', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        console.log('🧹 관리자 요청: 허수 프로젝트 전체 정리 시작...');
        // 1. 현재 프로젝트 목록 확인
        const projectsResult = await (0, connection_1.query)(`
    ...
    `);
        ...
        // 배치 삭제
        await (0, connection_1.query)('DELETE FROM projects WHERE id = ANY($1)', [phantomProjectIds]);
        // 4. 정리 후 상태 확인
        const finalResult = await (0, connection_1.query)('SELECT COUNT(*) as count FROM projects');
...
router.delete('/cleanup-test-data', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        console.log('🧹 관리자 요청: 테스트 데이터 정리 시작...');
        // 1. 현재 프로젝트 목록 확인
        const projectsResult = await (0, connection_1.query)('SELECT id, title, description, status, created_at FROM projects ORDER BY created_at DESC');
        console.log(`📊 현재 프로젝트 총 개수: ${projectsResult.rows.length}개`);
        // 2. 테스트/허수 데이터 식별
        ...
        await (0, connection_1.query)('DELETE FROM projects WHERE id = ANY($1)', [testProjectIds]);
        // 5. 정리 후 상태 확인
        const finalResult = await (0, connection_1.query)('SELECT COUNT(*) as count FROM projects');
        const remainingCount = parseInt(finalResult.rows[0].count);
...
router.get('/projects', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
...
router.post('/public-cleanup-phantom-projects', async (req, res) => {
    try {
