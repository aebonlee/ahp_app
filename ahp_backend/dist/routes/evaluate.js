 ...
 */
router.post('/direct', auth_1.authenticateToken, [
    (0, express_validator_1.body)('project_id').isInt().withMessage('Project ID must be an integer'),
    (0, express_validator_1.body)('target_key').isString().isLength({ min: 1 }).withMessage('Target key is required'),
    (0, express_validator_1.body)('value').isFloat({ min: 0.001 }).withMessage('Value must be positive'),
    (0, express_validator_1.body)('is_benefit').isBoolean().withMessage('is_benefit must be boolean')
], async (req, res) => {
    try {
        ...
        // 직접입력 데이터 저장 (UPSERT)
        const result = await (0, connection_1.query)(`INSERT INTO direct_entries (project_id, evaluator_id, target_key, value, is_benefit)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (project_id, evaluator_id, target_key)
         DO UPDATE SET value = $4, is_benefit = $5, updated_at = CURRENT_TIMESTAMP
         RETURNING *`, [project_id, evaluator_id, target_key, value, is_benefit]);
        // 진행상황 업데이트
 ...
 */
router.get('/direct/:projectId', auth_1.authenticateToken, async (req, res) => {
    try {
        const projectId = parseInt(req.params.projectId);
        const evaluatorId = req.user.userId;
        const entries = await (0, connection_1.query)(`SELECT de.*, c.name as criterion_name, a.name as alternative_name
         ...
         ORDER BY de.created_at DESC`, [projectId, evaluatorId]);
        res.json({
 ...
 */
router.post('/pairwise', auth_1.authenticateToken, [
    (0, express_validator_1.body)('project_id').isInt().withMessage('Project ID must be an integer'),
    (0, express_validator_1.body)('matrix_key').isString().withMessage('Matrix key is required'),
    (0, express_validator_1.body)('i_index').isInt({ min: 0 }).withMessage('i_index must be non-negative integer'),
    (0, express_validator_1.body)('j_index').isInt({ min: 0 }).withMessage('j_index must be non-negative integer'),
    (0, express_validator_1.body)('value').isFloat({ min: 0.111, max: 9 }).withMessage('Value must be between 1/9 and 9')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        await (0, connection_1.query)(`INSERT INTO pairwise_comparisons 
         (project_id, evaluator_id, matrix_key, i_index, j_index, value)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (project_id, evaluator_id, matrix_key, i_index, j_index)
         DO UPDATE SET value = $6, updated_at = CURRENT_TIMESTAMP`, [project_id, evaluator_id, matrix_key, i, j, adjustedValue]);
        // 진행상황 업데이트
        await updateEvaluatorProgress(project_id, evaluator_id);
        res.status(201).json({
 ...
 */
router.get('/pairwise/:projectId/:matrixKey', auth_1.authenticateToken, async (req, res) => {
    try {
        const projectId = parseInt(req.params.projectId);
        const matrixKey = req.params.matrixKey;
        const evaluatorId = req.user.userId;
        // 저장된 비교값 조회
        const comparisons = await (0, connection_1.query)(`SELECT i_index, j_index, value
         FROM pairwise_comparisons
         WHERE project_id = $1 AND evaluator_id = $2 AND matrix_key = $3`, [projectId, evaluatorId, matrixKey]);
        // 행렬 크기 결정
        const maxIndex = Math.max(...comparisons.rows.map(row => Math.max(row.i_index, row.j_index)), 0);
 ...
 */
router.post('/calculate/ahp', auth_1.authenticateToken, [
    ...
    try {
        ...
        if (result.consistencyRatio > 0.1) {
            suggestions = ahpCalculator_1.AHPCalculatorService.generateInconsistencyAdvice(matrix, result.weights, 3);
        }
        res.json({
            ...result,
            suggestions,
 ...
 */
router.post('/calculate/normalize', auth_1.authenticateToken, [
    (0, express_validator_1.body)('values').isArray().withMessage('Values must be an array'),
    (0, express_validator_1.body)('is_benefit').isBoolean().withMessage('is_benefit must be boolean')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        ...
        const normalizedWeights = ahpCalculator_1.AHPCalculatorService.normalizeDirectInput(values, is_benefit);
        res.json({
 ...
 * 평가자 진행상황 업데이트 헬퍼 함수
 */
async function updateEvaluatorProgress(projectId, evaluatorId) {
    try {
        // 총 평가 작업 수 계산 (기준 수 + 대안 수)
        const taskCount = await (0, connection_1.query)(`SELECT 
         (SELECT COUNT(*) FROM criteria WHERE project_id = $1) +
         (SELECT COUNT(*) FROM alternatives WHERE project_id = $1) as total_tasks`, [projectId]);
        // 완료된 작업 수 계산
        const completedCount = await (0, connection_1.query)(`SELECT 
         ...
         (SELECT COUNT(DISTINCT matrix_key) FROM pairwise_comparisons 
          WHERE project_id = $1 AND evaluator_id = $2) as completed_tasks`, [projectId, evaluatorId]);
        const totalTasks = taskCount.rows[0].total_tasks || 0;
        const completedTasks = completedCount.rows[0].completed_tasks || 0;
        const completionRate = totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0;
        // 진행상황 업데이트
        await (0, connection_1.query)(`INSERT INTO evaluator_progress 
       ...
       DO UPDATE SET 
         ...
         updated_at = CURRENT_TIMESTAMP`, [projectId, evaluatorId, totalTasks, completedTasks, completionRate, completionRate >= 100]);
    }
    catch (error) {
