 ...
 */
router.get('/excel/:projectId', auth_1.authenticateToken, async (req, res) => {
    try {
        const projectId = parseInt(req.params.projectId);
        const evaluatorId = req.user.userId;
        // 프로젝트 접근 권한 확인
        const projectCheck = await (0, connection_1.query)(`SELECT p.*, u.name as admin_name
       FROM projects p
...
async function collectProjectData(projectId) {
    // 기준 조회
    const criteriaResult = await (0, connection_1.query)(`SELECT * FROM criteria WHERE project_id = $1 ORDER BY level, "order"`, [projectId]);
    // 대안 조회
    const alternativesResult = await (0, connection_1.query)(`SELECT * FROM alternatives WHERE project_id = $1 ORDER BY "order"`, [projectId]);
    // 쌍대비교 데이터 조회
    const comparisonsResult = await (0, connection_1.query)(`SELECT pc.*, u.name as evaluator_name
     FROM pairwise_comparisons pc
     JOIN users u ON pc.evaluator_id = u.id
     WHERE pc.project_id = $1`, [projectId]);
    // AHP 결과 조회
    const resultsResult = await (0, connection_1.query)(`SELECT ar.*, u.name as evaluator_name
     FROM ahp_results ar
     JOIN users u ON ar.evaluator_id = u.id
     WHERE ar.project_id = $1`, [projectId]);
    // 최종 순위 조회
    const rankingResult = await (0, connection_1.query)(`SELECT * FROM aggregated_results WHERE project_id = $1`, [projectId]);
    // 평가자 조회
    const evaluatorsResult = await (0, connection_1.query)(`SELECT pe.*, u.name as evaluator_name, u.email
     FROM project_evaluators pe
     JOIN users u ON pe.evaluator_id = u.id
     WHERE pe.project_id = $1`, [projectId]);
    // 진행상황 조회
    const progressResult = await (0, connection_1.query)(`SELECT ep.*, u.name as evaluator_name
     FROM evaluator_progress ep
     JOIN users u ON ep.evaluator_id = u.id
     WHERE ep.project_id = $1`, [projectId]);
    // 평균 일관성 비율 계산
 ...
 */
router.get('/csv/:projectId', auth_1.authenticateToken, async (req, res) => {
    try {
        const projectId = parseInt(req.params.projectId);
        const { type = 'ranking' } = req.query;
        // 프로젝트 접근 권한 확인
        const projectCheck = await (0, connection_1.query)(`SELECT p.name FROM projects p
       WHERE p.id = $1 AND (p.admin_id = $2 OR EXISTS(
         SELECT 1 FROM project_evaluators pe 
         WHERE pe.project_id = p.id AND pe.evaluator_id = $2
        ...
        if (type === 'ranking') {
            // 최종 순위 CSV
            const rankingResult = await (0, connection_1.query)(`SELECT ar.final_ranking FROM aggregated_results ar WHERE ar.project_id = $1`, [projectId]);
