...
router.get('/:projectId', auth_1.authenticateToken, [
    ...
    try {
      ...
      `;
        ...
        const comparisonsResult = await (0, connection_1.query)(comparisonQuery, comparisonParams);
        res.json({
...
router.post('/', auth_1.authenticateToken, [
    ...
    (0, express_validator_1.body)('comparison_type').isIn(['criteria', 'alternatives']).withMessage('Comparison type must be criteria or alternatives'),
    (0, express_validator_1.body)('element_a_id').isInt().withMessage('Element A ID must be an integer'),
    (0, express_validator_1.body)('element_b_id').isInt().withMessage('Element B ID must be an integer'),
    (0, express_validator_1.body)('value').isFloat({ min: 0.111, max: 9 }).withMessage('Comparison value must be between 1/9 (0.111) and 9'),
    (0, express_validator_1.body)('parent_criteria_id').optional().isInt().withMessage('Parent criteria ID must be an integer')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const elementAResult = await (0, connection_1.query)(`SELECT id FROM ${elementTable} WHERE id = ? AND project_id = ?`, [element_a_id, project_id]);
        const elementBResult = await (0, connection_1.query)(`SELECT id FROM ${elementTable} WHERE id = ? AND project_id = ?`, [element_b_id, project_id]);
        ...
        // 기존 비교 확인
        const existingResult = await (0, connection_1.query)(`SELECT id FROM pairwise_comparisons 
         WHERE project_id = ? AND evaluator_id = ? AND comparison_type = ? 
         AND element_a_id = ? AND element_b_id = ? AND parent_criteria_id ${parent_criteria_id ? '= ?' : 'IS NULL'}`, parent_criteria_id
        ...
        if (existingResult.rows.length > 0) {
            // 기존 비교 업데이트
            result = await (0, connection_1.query)(`UPDATE pairwise_comparisons 
           SET value = ?, updated_at = CURRENT_TIMESTAMP
           WHERE id = ?`, [normalizedValue, existingResult.rows[0].id]);
            // 업데이트된 데이터 조회
            const updatedComparison = await (0, connection_1.query)('SELECT * FROM pairwise_comparisons WHERE id = ?', [existingResult.rows[0].id]);
            result = { rows: updatedComparison.rows };
        }
        else {
            // 새 비교 생성
            result = await (0, connection_1.query)(`INSERT INTO pairwise_comparisons 
           (project_id, evaluator_id, parent_criteria_id, comparison_type, element_a_id, element_b_id, value)
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [project_id, userId, parent_criteria_id || null, comparison_type, normalizedA, normalizedB, normalizedValue]);
            // 생성된 데이터 조회
            const createdComparison = await (0, connection_1.query)('SELECT * FROM pairwise_comparisons WHERE id = ?', [result.lastID]);
 ...
 */
router.get('/:projectId/matrix', auth_1.authenticateToken, [
    (0, express_validator_1.param)('projectId').isInt().withMessage('Project ID must be an integer')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        ...
        const elementsResult = await (0, connection_1.query)(elementsQuery, elementsParams);
...
router.delete('/:id', auth_1.authenticateToken, [
    (0, express_validator_1.param)('id').isInt().withMessage('Comparison ID must be an integer')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const checkResult = await (0, connection_1.query)(`SELECT pc.*, p.admin_id 
         FROM pairwise_comparisons pc
        ...
        // 비교 삭제
        await (0, connection_1.query)('DELETE FROM pairwise_comparisons WHERE id = ?', [id]);
        res.json({
 ...
 */
router.get('/:projectId/consistency', auth_1.authenticateToken, [
    (0, express_validator_1.param)('projectId').isInt().withMessage('Project ID must be an integer')
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
