...
router.get('/usage', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        // 현재 프로젝트 수 계산
        const projectCount = await (0, connection_1.query)(`
      SELECT COUNT(*) as count 
      FROM projects 
      WHERE admin_id = $1 AND status != 'archived'
    `, [userId]);
        // 평가자 수 계산
        const evaluatorCount = await (0, connection_1.query)(`
    ...
    `, [userId]);
        // 스토리지 사용량 계산 (근사치)
        const storageUsage = await (0, connection_1.query)(`
      SELECT 
    ...
    `, [userId]);
        // 할당량 정보
        const quotaInfo = await (0, connection_1.query)(`
...
// 사용기간 연장
router.post('/extend', auth_1.authenticateToken, [
    (0, express_validator_1.body)('days').isInt({ min: 1, max: 365 }).withMessage('Days must be between 1 and 365'),
...
// 데이터 초기화 (관리자용)
router.post('/reset-data', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        `, [userId]);
            ...
            const deletedProjects = await (0, connection_1.query)(`
...
// 관리자용: 만료된 trial 정리
router.post('/admin/cleanup-expired', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // 만료된 trial 비활성화
        const expiredTrials = await (0, connection_1.query)('SELECT deactivate_expired_trials()');
        // 완료된 프로젝트 아카이브
        const archivedProjects = await (0, connection_1.query)('SELECT archive_completed_projects()');
        // 만료된 아카이브 정리
        const cleanedArchives = await (0, connection_1.query)('SELECT cleanup_expired_archives()');
        // 오래된 로그 정리
        const cleanedLogs = await (0, connection_1.query)('SELECT cleanup_old_activity_logs()');
        res.json({
            success: true,
...
// 사용자 할당량 확인
router.get('/quota-check/:action', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
