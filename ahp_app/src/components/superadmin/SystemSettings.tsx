...
const SystemSettings: React.FC = () => {
  ...
  const testEmailConnection = async () => {
    ...
    try {
      ...
      showActionMessage('success', '테스트 이메일이 전송되었습니다.');
    } catch (error) {
      showActionMessage('error', '이메일 전송 실패. SMTP 설정을 확인하세요.');
    } finally {
  ...
  const importSettings = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setHasChanges(true);
        showActionMessage('info', '설정을 가져왔습니다. 저장 버튼을 클릭하여 적용하세요.');
      } catch (error) {
        showActionMessage('error', '잘못된 설정 파일입니다.');
      }
    };
