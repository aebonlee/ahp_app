...
# Render 프로덕션 환경에서 자동으로 제공되는 DATABASE_URL 사용
# DATABASE_URL이 없으면 fallback PostgreSQL 사용 (프로덕션 전용)
DATABASE_URL = os.environ.get('DATABASE_URL')

if DATABASE_URL:
    # Render에서 자동 제공하는 DATABASE_URL 파싱
    DATABASES = {
        'default': dj_database_url.parse(DATABASE_URL, conn_max_age=600)
    }
else:
    # Fallback PostgreSQL 설정 (프로덕션 전용, 로컬 무시)
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.postgresql',
            'NAME': 'ahp_app_db',
            'USER': 'ahp_app_user', 
            'PASSWORD': 'ahp_app_password',
            'HOST': 'dpg-placeholder.render.com',
            'PORT': '5432',
        }
    }
