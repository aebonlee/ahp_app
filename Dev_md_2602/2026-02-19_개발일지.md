...
## 세션 26 — hardcoded localhost 완전 제거 + /api/v1/ 경로 전체 수정

### 작업 배경

세션 25 이후 추가 감사에서 발견한 문제들:
1. `subscriptionService.ts`와 `ModelBuilder.tsx`에 하드코딩된 `localhost:8000` URL이 남아있음
2. `systemHealthCheck.ts`, `connectionTest.ts`, `projectDebugger.ts`에 `/api/v1/` 레거시 경로 잔존
3. `App.tsx` 프로젝트 삭제 실패 시 `window.alert()` 하나 남아있음

### 수정 1: subscriptionService.ts

**파일**: `src/services/subscriptionService.ts`

| 변경 전 | 변경 후 |
|---------|---------|
| `private baseUrl` 필드 + constructor 내부 `process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '...'` | 필드/constructor 제거 |
| `fetch(\`${this.baseUrl}...\`)` | `import { API_BASE_URL } from '../config/api'` + `fetch(\`${API_BASE_URL}...\`)` |

### 수정 2: ModelBuilder.tsx

**파일**: `src/components/model/ModelBuilder.tsx`

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| API URL | `const API_BASE_URL = process.env.NODE_ENV === 'development' ? 'http://localhost:8000' : '...'` (로컬 선언) | `import { API_BASE_URL } from '../../config/api'` |
| 기준 조회 | `/api/v1/criteria/` | `/api/service/projects/criteria/?project=${id}` |
| 대안 조회 | `/api/v1/alternatives/` | `/api/service/projects/criteria/?type=alternative` |
| 프로젝트 조회 | `/api/projects/${id}` | `/api/service/projects/projects/${id}/` |
| 응답 파싱 | `projectData.project` (중첩 접근) | `projectData` (flat 응답) |
| 기준 배열 | `criteriaData.criteria \|\| []` | `criteriaData.results ?? criteriaData.criteria ?? (Array.isArray(criteriaData) ? criteriaData : [])` |

### 수정 3: App.tsx window.alert() 제거
...
### 수정 4: 유틸리티 파일 /api/v1/ → /api/service/ 수정

| 파일 | 수정 전 | 수정 후 |
|------|---------|---------|
| `utils/systemHealthCheck.ts` (3곳) | `/api/v1/status/`, `/api/v1/projects/` 등 | `/api/service/status/`, `/api/service/projects/projects/` 등 |
| `utils/connectionTest.ts` | `/api/v1/projects/projects/` (replace_all) | `/api/service/projects/projects/` |
| `utils/projectDebugger.ts` | `/api/v1/status/` | `/api/service/status/` |

### 검증

```
grep -r 'localhost:8000' src/  → 0건
grep -r 'api/v1/' src/        → 0건 (프로덕션 코드 기준)
grep -r 'window\.alert' src/  → 0건
```
...
Compiled successfully.
File sizes after gzip:
  165.41 kB  build/static/js/main.*.js
ESLint 경고: 0개
```
...
| 레포 | 커밋 | 내용 |
|------|------|------|
| 프론트 | `e5276a4` | fix: hardcoded localhost 제거 및 /api/v1/ 경로 수정 |

> 변경 규모: 6개 파일, +38 / -53 lines

---

## 세션 27 — 전체 코드베이스 console.log/error/warn 전면 정리

### 작업 배경

세션 26 이후 전체 코드베이스를 grep한 결과 아직 `console.log` 약 700개+가 144개 파일에 잔존.
세션 25에서 주요 파일만 정리했으나 대부분의 components, hooks, services가 미처리 상태.
...
|------|----------|
| `console.log(...)` | 모두 제거 |
| catch 블록 `console.error(...)` — `showActionMessage`/`setError`/`throw` 등 다른 오류 처리 있음 | 제거 |
| catch 블록 `console.error(...)` — 오류 처리 수단이 이것뿐임 | 유지 (emoji만 제거) |

### 처리 방식

8개 background agent를 병렬 실행하여 파일군별로 분담:

| 에이전트 | 담당 파일군 |
|---------|-----------|
| Agent 1 (afce703) | `App.tsx` |
| Agent 2 (a439d63) | services: `authService`, `sessionService`, `anonymousEvaluationService`, `dataService` |
| Agent 3 (a632cde) | utils: `realTimeSync`, `exportService`, `fileUploadService`, `djangoAdminService`, `systemManagementService`, `api.ts`, `ahpApiService` |
| Agent 4 (a837512) | auth/evaluation 컴포넌트: `EnhancedAuthFlow`, `AnonymousEvaluator`, `EventSequenceDemo`, `RealUserManagement` |
| Agent 5 (a3a9d88) | settings/admin/ai-management 컴포넌트 (18개) |
| Agent 6 (aa7ad8b) | evaluator 컴포넌트 (12개) |
| Agent 7 (a6e5e9f) | AI pages, analysis 컴포넌트 (15개) |
| Agent 8 (ae04d0e + a11a030) | collaboration/criteria/workflow/misc 컴포넌트, hooks, 나머지 파일 |

### 주요 처리 결과

| 파일 | 제거 | 유지 |
|------|------|------|
| `App.tsx` | 67개 | 12개 (catch 유일 오류 신호) |
| `authService.ts` | 9개 | 2개 |
| `sessionService.ts` | 11개 | 2개 |
| `anonymousEvaluationService.ts` | 13개 | 5개 |
| `dataService.ts` | 10개 | 12개 |
| `api.ts` | 전체 | 1개 (outer catch) |
| `realTimeSync.ts` | 13개 | 8개 |
| `EnhancedAuthFlow.tsx` | 16개 | 0 |
| `AnonymousEvaluator.tsx` | 15개 | 3개 |
| settings/admin (18개 파일) | 35개 | 26개 |
| evaluator (12개 파일) | 16개 | 15개 |
| AI pages/analysis (15개 파일) | 20개 | 23개 |
| collaboration/criteria/workflow/misc | 전체 정리 | — |
| hooks/auth/project | 16개 | 15개 |

> 참고: `connectionTest.ts`, `projectDebugger.ts`, `systemHealthCheck.ts`는 의도적 디버그 도구이므로 유지.
...
| 레포 | 커밋 | 내용 |
|------|------|------|
| 프론트 | `25dc599` | refactor: console.log 전면 정리 - 프로덕션 코드 정리 |

> 변경 규모: 56개 파일, +205 / -734 lines (순감 -529 lines)
...
| 우선순위 | 항목 | 비고 |
|---------|------|------|
| MEDIUM | `window.confirm()` 36개+ → 커스텀 모달 | UX 개선, 추후 처리 |
| MEDIUM | SystemReset 실제 backend API | 백엔드 엔드포인트 없음 |
| LOW | 백엔드 SMTP | 이메일 서비스 계약 후 |
| LOW | KG이니시스 PG | 사업 준비 후 |
