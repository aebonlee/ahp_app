...
| CORS evil.com 허용 | ⚠️ ACAO 반환 | ✅ 차단 예정 (Render 재배포 후) |
...
2. **CORS 수정 Render 반영 확인** (배포 후 evil.com 재테스트)

### 우선순위 MEDIUM
3. **인증 흐름 E2E 테스트**: 프론트엔드에서 실제 로그인→프로젝트생성→평가 흐름 실행

### 우선순위 LOW
4. **Render.com PostgreSQL → Neon.tech 이전** ($7/월 절약)
5. **새 프로덕션 레포** 생성
